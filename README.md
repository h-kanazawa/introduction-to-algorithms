# Notebook: Introduction to Algorighms

This is my notebook when I read [*Introduction to Algorithms, Third Edition*](https://mitpress.mit.edu/books/introduction-algorithms).

## 2

[Insertion Sort](src/insertion_sort.py)
[Selection Sort](src/selection_sort.py)
[Merge Sort](src/merge_sort.py)

## 4

[Max Subarray](src/max_subarray.py)
[Strassen Algorithm](src/square_matrix_multiply)

## 5

[Permute by Random](src/random_permute.py)

## 6

[Heap Sort](src/heap_sort.py)

## 7

[Quick Sort](src/quick_sort.py)

## 8

[Counting Sort](src/counting_sort.py)

## 9

[Minimum and Maximum](src/minimum.py)

## 10

## 11

[Hash Table](src/hash_table.py)

## 12

[Binary Search Tree](src/binary_search_tree.py)

## 13

[Red Black Tree](src/red_black_tree.py)

## 14

## 15

[Cut Rod](src/cut_rod.py)
[Matrix Chain Order](src/matrix_chain_order.py)
[Longest Common Subsequence](src/lcs.py)
[Optimal Binary Search Tree](src/optimal_bst.py)

## 16

[Greedy Activity Selector](src/recursive_activity_selector.py)

## 17

## 18

[B Tree](src/b_tree.py)

## 19

[Fibonacci Heap](src/fib_heap.py)

## 20

[van Emde Boas Tree](src/van_emde_boas_tree)

## 21

[Disjoing Set](src/disjoint_set.py)

## 22

[Breadth First Search](src/bfs.py)
[Depth First Search](src/dfs.py)

## 23

[Minimum Spanning Tree](src/minimum_spanning_tree.py)

## 24

[Bellman-Ford Algorighm](src/bellman_ford.py)
[Dijkstra](src/dijkstra.py)

## 25

[All Pairs Shortest Path](src/all_pairs_shortest_path.py)

## 31

[GCD, Euclidean Algorithm](src/euclid.py)
[Chinese remainder theorem](src/chinese_remainder_theorem.py)
[Modular Exponentiation](modular_exponentiation.py)

## 32

[String Matching, Rabin-Karp](src/string_matching.py)
[Finite Automaton Matcher](src/finite_automaton_matcher.py)

## 33

[Segments Intersect Detection](src/geometry.py)
